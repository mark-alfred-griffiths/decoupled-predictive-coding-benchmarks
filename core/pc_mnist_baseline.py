#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Train a vanilla predictive coding network on MNIST or CIFAR-10 with JPC."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--preset", choices=["default", "mnist_bottleneck"], default="default",
                        help="Use a named hyperparameter preset (mnist_bottleneck fixes the width to 128).")
    ...
    parser.add_argument("--depth", type=int, default=4, help="Number of layers including the output layer.")
    ...
    parser.add_argument("--input-noise-level", type=float, default=0.0,
                        help="Strength of input corruption. Interpreted as σ for Gaussian noise, total flip probability for salt-and-pepper noise, or occlusion block side as a fraction of the image size.")
...
def set_jax_platform(device_choice: str) -> None:
    if device_choice == "cpu":
        ...
        # Allow callbacks that require a host device (e.g. jax.pure_callback) to
        # place their work on the CPU, even when running primarily on the GPU.
        # Without listing "cpu" as a fallback platform, JAX initialises without a
        # host device and certain callbacks fail at runtime.
...
args = parse_args()
if args.preset == "mnist_bottleneck":
    # Keep the depth configurable, but snap the width to the 128-wide bottleneck setting
    # used in the MNIST narrow-model experiments.
    args.width = 128
...
try:
    ...
    raise SystemExit(
        "PyTorch and torchvision are required to run pc_mnist_baseline.py."
    ) from exc
...
def prepare_dataloaders(batch_size: int, eval_batch_size: int, data_root: str, dataset: str,
                        noise_type: str, noise_level: float) -> Tuple[DataLoader, DataLoader]:
    if dataset == "mnist":
        transform_steps = [transforms.ToTensor(), transforms.Normalize((0.1307,), (0.3081,))]
...
def main() -> None:
    ...
    input_dim = 28 * 28 if args.dataset == "mnist" else 32 * 32 * 3
    output_dim = 10
    ...
    for step in range(1, args.train_steps + 1):
        ...
        metrics = {
            ...
            # Metadata to keep downstream comparators from losing context.
            "preset": args.preset,
            "dataset": args.dataset,
            "width": args.width,
            "depth": args.depth,
